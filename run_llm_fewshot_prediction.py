...
from agents import LLMFewShotPredictionAgent
...
def process_sample(sample_index, all_labels, all_data, fewshot_agent, train_indices, num_shots, LABEL_MEANINGS, column_names):
    """
    Processes a single test sample for few-shot LLM prediction.
    """
    # Prepare test data text
    raw_data_sample = all_data[sample_index]
    test_text_data = format_timeseries_data(raw_data_sample, column_names)

    # Select few-shot examples
    shot_indices = random.sample(train_indices, num_shots)
    few_shot_examples = []
    for shot_id in shot_indices:
        shot_data_sample = all_data[shot_id]
        shot_text_data = format_timeseries_data(shot_data_sample, column_names)
        shot_label = all_labels[shot_id]
        shot_label_meaning = LABEL_MEANINGS.get(shot_label, "Unknown")
        
        few_shot_examples.append({
            "id": shot_id,
            "text_data": shot_text_data,
            "label_meaning": shot_label_meaning
        })

    logging.info(f"Generating few-shot LLM prediction for sample_index: {sample_index} using {len(few_shot_examples)} shots.")
    response_str = fewshot_agent.execute(test_text_data=test_text_data, examples=few_shot_examples)

    result = parse_json_from_string(response_str)
    if not result:
        result = {"prediction": -1, "reasoning": "Few-shot LLM prediction failed."}
...
def main(args):
    """
    Main function to run few-shot LLM predictions on the test set.
    """
    ...
    fewshot_agent = LLMFewShotPredictionAgent(model_name=args.prediction_model, dataset_name=DATASET_NAME)

    final_results = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        future_to_id = {
            executor.submit(process_sample, idx, all_labels, all_data, fewshot_agent, train_indices, args.num_shots, LABEL_MEANINGS, COLUMN_NAMES): idx
            for idx in test_indices
        }

        for future in tqdm(concurrent.futures.as_completed(future_to_id), total=len(test_indices), desc="Running Few-Shot LLM Predictions"):
            try:
    ...
    logging.info(f"Few-shot LLM prediction results saved to {output_path}")

    evaluate_results(results_df, LABEL_MEANINGS, DATASET_NAME)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Few-shot LLM predictions on a time-series dataset.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Name of the LLM for few-shot prediction.'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--output_file', type=str, default='llm_fewshot_predictions.csv',
