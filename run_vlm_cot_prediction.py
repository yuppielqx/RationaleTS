...
from agents import ImageZeroCOTPredictionAgent
...
def evaluate_results(results_df: pd.DataFrame, LABEL_MEANINGS: dict, dataset_name: str):
    ...
    if num_classes > 1:
        ...
        if y_true_binarized.shape[1] == y_pred_binarized.shape[1]:
            ...
            logging.warning(
                "Could not compute AUROC. Shape mismatch between true and predicted labels after binarization.")

    logging.info("\nClassification Report:\n" + classification_report(y_true, y_pred,
                                                                      target_names=[LABEL_MEANINGS[i] for i in
                                                                                    sorted(LABEL_MEANINGS.keys())],
                                                                      zero_division=0))
...
def main(args):
    ...
    # --- 3. Initialize Agent ---
    direct_prediction_agent = ImageZeroCOTPredictionAgent(model_name=args.prediction_model, dataset_name=DATASET_NAME,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        for future in tqdm(concurrent.futures.as_completed(future_to_id), total=len(test_indices),
                           desc="Running Direct VLM Predictions"):
            try:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--output_file', type=str, default='vlm_CoT_predictions.csv',
