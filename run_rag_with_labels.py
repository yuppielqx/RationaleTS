...
from agents import AnalysisAgent, RAGWithLabelsAgent
...
def process_sample(image_id, all_labels, embedding_handler, analysis_agent, rag_prediction_agent, IMAGE_DIR, top_k, retrieval_mode='text', hybrid_alpha=0.5, all_data_values=None, train_data_embeddings=None, query_embedding_tabpfn=None, LABEL_MEANINGS=None):
    ...
    logging.info("Executing RAG prediction with example labels...")
...
def main(args):
    ...
    rag_prediction_agent = RAGWithLabelsAgent(model_name=args.prediction_model, dataset_name=DATASET_NAME, image_url_format='dict')
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        for future in tqdm(concurrent.futures.as_completed(future_to_id), total=len(test_indices), desc="Running RAG w/ Labels"):
            try:
    ...
    logging.info(f"RAG with labels prediction results saved to {output_path}")

    evaluate_results(results_df, LABEL_MEANINGS)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run RAG predictions with example labels.")
    ...
    parser.add_argument('--output_file', type=str, default='rag_with_labels_predictions.csv')
