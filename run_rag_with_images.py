...
from agents import AnalysisAgent, RAGWithImagesAgent
...
def process_sample(image_id, all_labels, embedding_handler, analysis_agent, rag_prediction_agent, IMAGE_DIR, top_k, retrieval_mode='text', hybrid_alpha=0.5, all_data_values=None, train_data_embeddings=None, query_embedding_tabpfn=None):
    ...
    if retrieval_mode == 'hybrid':
        ...
        # For 'worst' mode compute the same hybrid scores but select the lowest-K examples
        sims_tabpfn = cosine_similarity(query_embedding_tabpfn.reshape(1, -1), train_data_embeddings)[0]
        query_embedding_text = embedding_handler.get_embeddings([query_text])
        sims_text = cosine_similarity(query_embedding_text, embedding_handler.coarse_embeddings)[0]
        hybrid_scores = hybrid_alpha * min_max_scale(sims_tabpfn) + (1 - hybrid_alpha) * min_max_scale(sims_text)
    ...
    logging.info("Executing RAG prediction with example images...")
...
def main(args):
    ...
    if args.retrieval_mode in ['tabpfn', 'hybrid', 'worst']:
        ...
        # This part is copied from run_rag_prediction.py and can be refactored
        if os.path.exists(TRAIN_EMB_PATH) and os.path.exists(TEST_EMB_PATH):
            ...
            # Placeholder for generation logic if files are missing
    ...
    rag_prediction_agent = RAGWithImagesAgent(model_name=args.prediction_model, dataset_name=DATASET_NAME, image_url_format='dict')
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        for future in tqdm(concurrent.futures.as_completed(future_to_id), total=len(test_indices), desc="Running RAG w/ Images"):
            try:
    ...
    logging.info(f"RAG with images prediction results saved to {output_path}")

    evaluate_results(results_df, LABEL_MEANINGS)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run RAG predictions with example images.")
    ...
    parser.add_argument('--retrieval_mode', type=str, default='hybrid', choices=['text', 'tabpfn', 'hybrid', 'worst'])
    parser.add_argument('--hybrid_alpha', type=float, default=0.2)
    parser.add_argument('--output_file', type=str, default='rag_with_images_predictions.csv')
