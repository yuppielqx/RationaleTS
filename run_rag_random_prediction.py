...
def process_sample(image_id, all_labels, embedding_handler, analysis_agent, rag_prediction_agent, IMAGE_DIR, top_k, retrieval_mode='text', hybrid_alpha=0.5, all_data_values=None, train_data_embeddings=None, query_embedding_tabpfn=None):
    ...
    if retrieval_mode == 'random':
        logging.info(f"Randomly selecting {top_k} examples for image_id: {image_id}")
        similar_examples = embedding_handler.rag_metadata.sample(n=top_k).to_dict('records')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--retrieval_mode', type=str, default='random', choices=['text', 'tabpfn', 'hybrid', 'random'],
        help='Retrieval mode: "text" (semantic), "tabpfn" (raw value embedding), "hybrid" (weighted), or "random".'
    )
    parser.add_argument(
        '--hybrid_alpha', type=float, default=0.4,
        help='Weight for TabPFN similarity in hybrid mode (0.0 to 1.0). Text similarity weight is (1 - alpha).'
    )
    parser.add_argument(
        '--output_file', type=str, default='rag_random_predictions.csv',
